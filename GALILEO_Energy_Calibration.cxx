...
GALILEO_Energy_Calibration::GALILEO_Energy_Calibration(){
...
void GALILEO_Energy_Calibration::load_Calibration_File(){
    
    const char* format = "%d %lf %lf %lf %lf %lf";


    ifstream calib_file("Configuration_Files/GALILEO_Energy_Calibration.txt");

    if(calib_file.fail()){
        cerr << "Could not find GALILEO Energy Calibration file" << endl;
        exit(0);
    }
    string line;
...
double GALILEO_Energy_Calibration::Calibrate_FEBEX_E(double E,int det_id){
